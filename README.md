...
On small screens, the hamburger button opens **Edit wheel**, **Share**, and **Spin history**. Selecting an action, pressing Escape, or clicking outside closes the menu. About and Mute stay in the header.
...
- Storage failures leave the wheel usable for the session and announce a status to screen readers. Stored labels are trimmed, uppercased, and normalized to NFC before validation, using the same 12 UTF-16-unit limit as the editor. Invalid stored configurations fall back to the default wheel.
...
Longer holds increase launch energy for the same random preload, and even the weakest full-charge launch is stronger than the strongest zero-charge launch. The preload range grows with charge to keep travel varied under the stronger brake. The randomized launch speed remains below 38 rad/s across the full charge range. New records use simulation version 8, which gates pointer forces and impacts on physical peg contact. Each launch resets transient contact state and records its seed, charge, starting angle, complete choice list, physics settings, and simulation version.

**Replay** in Spin history restores that configuration and reproduces a saved spin. A shared replay that is not already saved appears there as **Replay shared spin**. It remains available after editing choices. Completed replay records persist in Spin history; share links can also carry them across reloads. Replay requires the same simulation version. Version 8 removes pointer forces before peg contact, so earlier-version replays are unavailable; their shared choices still load. The physics-only replay suite verifies exact state and impact equivalence across Chrome, Firefox, and WebKit for 101 scenarios. The [recorded results](docs/cross-browser-replay.json) identify the tested browser builds and platform.
...
The [current report](docs/distribution.json) contains version 8 durations and outcome frequencies. Across the sampled scenarios, shorter-hold means range from 8.04 to 13.74 seconds and full-charge means from 19.46 to 24.65 seconds. Version 8 removes approach-zone drag; both duration checks allow 50-choice averages below 14 seconds for shorter holds and 26 seconds for full charge. The other sampled wheel sizes retain limits of 12.5 and 23 seconds. The historical [duration comparison](docs/duration-comparison.json) records the earlier version 2–3 shortening, before full-charge spins gained their longer coast.

## Outcome distribution

Equal segment sizes do **not** guarantee equal winning probabilities. Outcomes depend on charge, starting position, and physical parameters. The diagnostic samples 1,024 seeds per scenario for 2/8/40/50 choices. Low charges of 0/5/10/25% each use five starting offsets: the original 0 and 0.37 radians, plus quarter-, half-, and three-quarter-segment offsets from the default angle. This includes a peg directly under the pointer at the half-segment offset. The 55% and 100% scenarios retain the original two offsets. That makes 96 scenarios and 98,304 spins under the default physics settings. The same seed recipe is retained across scenarios for reproducible comparisons; the report records the coverage and exact offsets. Each scenario is reported separately so pooling cannot hide a biased charge or starting position.

The checked-in [sample results](docs/distribution.json) describe simulation version 8. All 98,304 sampled spins settled; the largest per-scenario total variation distance was 0.1072, below the 0.2 regression limit. The 80 low-charge scenarios (0–25%) account for 81,920 spins; their maximum distances were 0.0381 / 0.0449 / 0.1072 / 0.1021 for 2/8/40/50 choices respectively. The most uneven two-choice low-charge split was 473–551. These remain finite-sample measurements, not proof of exact uniformity. The [duration comparison](docs/duration-comparison.json) compares versions 2 and 3 using identical seed inputs and scenarios, separate from the smaller exploratory sample used to tune the brake.
...
Browser tests use locally installed Google Chrome with WebGPU enabled. They cover keyboard and pointer cancellation, locked controls, full-speed pointer motion reaching the GPU, replay, bulk editing, 30-entry history retention and restoration, persistence failures, unsupported WebGPU, desktop/mobile resizing, idle rendering, and injected device-loss recovery. Active-spin recovery tests destroy the native GPU device, exercise a failed retry followed by success, and inject page-visibility signals. They verify charging cancellation, frozen physics while paused, agreement with an uninterrupted reference spin, restored controls, and unchanged history after an interrupted replay. The distribution diagnostic is separate from the fast suite and prints per-scenario histograms and concentration metrics. It also checks settling, guards against the old severe outcome concentration, and checks short-hold averages below 12.5 seconds (14 for 50 choices), the 95th percentile below 16 seconds, and every sampled short-hold spin below 20 seconds. Full-charge spins instead target a mean above 15 seconds and below 23 seconds (26 for 50 choices), a 95th percentile below 27 seconds, and all sampled spins below 30 seconds.
