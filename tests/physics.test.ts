...
function run(seed: number[], charge: number, count = 8, maxSeconds = 45) {
  ...
  for (let i = 0; i < maxSeconds / FIXED_DT && !engine.isSettled(); i++) engine.step(FIXED_DT);
  return { engine, impacts, result: selectedIndex(engine.wheel.angle, count) };
}
...
describe("mechanical wheel physics", () => {
  ...
  it("turns a longer hold into substantially more launch energy", () => {
    ...
    expect(high.wheel.angularVelocity).toBeGreaterThan(low.wheel.angularVelocity * 4);
  });
  ...
  it.each([2, 8, 50])("remains stable and settles with %i segments", count => {
