...
test('automatically stores 30 JSON snapshots and replays directly from history after reload', async ({ page }) => {
  ...
  await page.evaluate(version => {
    localStorage.setItem('momentum-spin-history', JSON.stringify(Array.from({ length: 30 }, (_, i) => ({
  ...
  expect(history[29].completedAt).toBe(972);
  await closeEditor(page);
  ...
  await expect(page.locator('#spin-history li')).toHaveCount(30);
  ...
  await expect(page.locator('.spin-controls button')).toHaveCount(1);
  ...
  await page.keyboard.press('Space');
  await expect(page.locator('#result')).toHaveClass('winner', { timeout: 45_000 });
  const nextHistory = await page.evaluate(() => JSON.parse(localStorage.getItem('momentum-spin-history')!));
  expect(nextHistory).toHaveLength(30);
  expect(nextHistory[0].completedAt).toBeGreaterThan(history[0].completedAt);
  expect(nextHistory.slice(1)).toEqual(history.slice(0, 29));
});
